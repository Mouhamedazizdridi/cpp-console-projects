...
enum enMainMenueOptions {
	eShowClientList = 1, eAddClient = 2, eDeleteClient = 3, eUpdateClient = 4, eFindClient = 5, eTransactions=6 ,eExit = 7
};
enum enTransactionsMenueOptions {
	eDeposit=1, eWithdraw=2, eTotalBalance=3, eBack=4
};
struct sClientInfo {
...
double ReadAmount() {
	double Amount;
	cout << "please enter Amount to deposit ? "; cin >> Amount;
	return Amount;
}
int ReadMainMenueOption() {

	int option;
	cout << "choose what u want to do ? [1 to 7]? "; cin >> option;
	return option;
}
int ReadTransactionsMenueOption() {
	int option;
	cout << "choose what u want to do ? [1 to 4]? "; cin >> option;
	return option;
}
...
bool DepositAmount(string AccNumber, vector<sClientInfo> vClients) {
	sClientInfo client;
	char answer;
	if (FoundClientByAccountNumber(AccNumber, vClients, client)) {
		PrintClientInfo(client);
...
double ReadAmountToWithdraw(double AccBalance) {
	double Amount;
	do { cout << "please enter Amount to withdraw ? "; cin >> Amount; } while (AccBalance<Amount);
	return Amount;
}
bool WithdrawAmount(string AccNumber, vector<sClientInfo> vClients) {
	sClientInfo client;
	char answer;
	if (FoundClientByAccountNumber(AccNumber, vClients, client)) {
		PrintClientInfo(client);
		double Amount = ReadAmountToWithdraw(client.AccountBalance);
		cout << "\nAre you sure to Withdraw ? y/n? "; cin >> answer;
		if (answer == 'y' || answer == 'Y') {
			client.AccountBalance -= Amount;
			SaveToVector(vClients, client, AccNumber);
			SaveDataToFile(FileName, vClients);
			cout << "Withdraw succesfuly\n";
			vClients = LoadClientsDataFromFile(FileName);
		}
		return true;
	}
...
double CalculateTotalBalance(vector<sClientInfo> vClients) {
	double Total = 0.0;
	for (sClientInfo& c : vClients) {
...
void Deposit() {
	...
	DepositAmount(AccNumber, vClients);
}
...
void Withdraw() {
	...
	string AccNumber = ReadAccountNumber();
	vector<sClientInfo> vClients = LoadClientsDataFromFile(FileName);
	WithdrawAmount(AccNumber, vClients);
}
...
bool PerfromTransactionMenueOption(enTransactionsMenueOptions choice) {
	switch (choice) {
	case eDeposit:
		system("cls");
		Deposit();
...
void ShowTransactionMenue() {
	while (true) {
		...
		enTransactionsMenueOptions choice = (enTransactionsMenueOptions)ReadTransactionsMenueOption();
		if (PerfromTransactionMenueOption(choice)) {
			break; 
		}
	}
}
